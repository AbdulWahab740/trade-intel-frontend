  ...
  return new Promise((resolve, reject) => {
    Papa.parse(text, {
      ...
      complete: (results) => {
        ...
        // Keep rows that have core fields and ANY non-zero numeric in quantity or value
        resolve(results.data.filter(row => {
          const hasCore = row.Month && row.Group && row.Commodity;
          const hasQuantity = Number(row.Quantity_2025) > 0 || Number(row.Quantity_2024) > 0;
          const hasPKR = Number(row.Rupees_2025) > 0 || Number(row.Rupees_2024) > 0;
          const hasUSD = Number(row.Dollar_2025) > 0 || Number(row.Dollar_2024) > 0;
          return hasCore && (hasQuantity || hasPKR || hasUSD);
        }));
      },
